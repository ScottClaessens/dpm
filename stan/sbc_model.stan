...
generated quantities{
  real log_likelihood = 0; // log-likelihood, not pointwise
  matrix[N_obs,J] yrep; // predictive checks
  {
    ...
    for (i in 1:N_obs) {
      ...
      yrep_temp[i,2] = bernoulli_logit_rng(eta[tip_id[i]][2] + drift_tips[tip_id[i]][2]);
      
      if (prior_only == 1) {
        log_likelihood += normal_lpdf(yrep_temp[i,1] | eta[tip_id[i]][1], sigma_tips[tip_id[i]][1]);
        log_likelihood += bernoulli_logit_lpmf(to_int(yrep_temp[i,2]) | eta[tip_id[i]][2] + drift_tips[tip_id[i]][2]);
      }
      
      else {
        log_likelihood += normal_lpdf(y[i,1] | eta[tip_id[i]][1], sigma_tips[tip_id[i]][1]);
        log_likelihood += bernoulli_logit_lpmf(to_int(y[i,2]) | eta[tip_id[i]][2] + drift_tips[tip_id[i]][2]);
      }
    }
  yrep = yrep_temp;
  }
