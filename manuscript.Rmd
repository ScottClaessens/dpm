---
title: "Causal inference and the detection of coevolutionary contingencies using generalized dynamic phylogenetic models"
...
\begin{center}
Phylogenetic comparative methods are widely used to study the coevolution of biological or cultural traits over evolutionary time. Among the most popular of these methods are phylogenetic generalised linear mixed models, phylogenetic path analysis, and Pagel's `discrete' method. While these methods are appropriate under specific scenarios, they are also limited by often unrealistic assumptions about the evolutionary process and limits on the number and type of traits that researchers can include. Here, we develop a novel class of \textit{generalized dynamic phylogenetic models} (GDPMs) that overcomes these limitations and synthesizes the strengths of existing methods into a generalizable framework for dynamic phylogenetic inference. Treating the phylogeny as an implicit time series, GDPMs model the co-evolution of any number of traits undergoing both selection and drift, capable of inferring directed evolution (X -> Y vs Y -> X), feedback (X <-> Y), and contigencies (e.g., first X, then Y). We introduce the \texttt{coevolve} R package, a user-friendly package purpose-built for fitting GDPMs in a Bayesian framework using Stan, accompanied by a short tutorial. We use synthetic data and simulation-based calibration to show that our models can recover true parameter values and yield unbiased estimates. We also present several empirical applications of GDPMs, including analyses of brain size in non-human primates and social complexity in human societies. These examples highlight the flexibility of the Bayesian framework, allowing researchers to model latent variables, mulitlevel structures and repeated measures, measurement error, missing data, and other complexities inherent in comparative data.
\end{center}
...
introduce a novel class of *generalized dynamic phylogenetic models* (GDPMs). We provide a 
...
statistical inference [@felsenstein1985phylogenies; @harvey1998phylogenetic]. Nonetheless, adjustment for phylogeny is not a 
magic fix for all sources of unobserved confounding, nor does it guarantee that 
resulting estimates are causally interpretable [@Hansen2014]. Therefore, while all 
...
Brownian motion, which assumes that the trait variance for a given species or population is proportional to the total 
branch length of the tree leading from root to tip, and the covariance between 
any two species is proportional to their amount of shared history [@felsenstein1985phylogenies]. This process
...
of adaptive radiation allow the rate of change to decrease over time [@harmon2010early], 
while Ornstein-Uhlenbeck models characterize evolutionary change as the product
of both stochastic and deterministic processes [@Hansen2014; @butler2004phylogenetic].
...
To date, Pagel's [-@Pagel1994] "discrete" method is the most widely-used application
...
morphometrics [@zelditch2012geometric], environmental quality and climate metrics in ecology
[@arhonditsis2006exploring], life history variation [@Stott2024], and canonical axes of 
correlational selection in quantitative genetics [@posthuma2003theory]. While latent 
...
phylogenetic principal component analysis to apply Pagel's method
...
## Dynamic methods with continuous traits

There are several other approaches to dynamic causal inference, based on the Ohrnstein-Uhlenbeck (OU) model, that relax Pagel's assumption of discrete traits. 
...
[@vasicek1977equilibrium]). In evolutionary biology, the mean-reverting quality is loosely 
interpreted as "selection" and accordingly the Gaussian noise is labeled "drift"
[@uhlenbeck1930theory; @hansen1997stabilizing; @Hansen2014]. The basic OU form is
...
[@hansen1997stabilizing]). These approaches exploit the fact that, if selection regimes are 
...
matrix of a trait. Butler and King [@butler2004phylogenetic] provided a maximum-likelihood 
algorithm for this approach, and a Bayesian implementation was developed by 
Ross et al. [@ross2016origins]. These approaches assume that one trait influences the evolution of another, 
but not vice versa (i.e. no feedback). The multivariate OU model was further extended to allow the estimation of reciprocal, directed evolution between traits [@bartoszek2012phylogenetic], a method that has been implemented in the mvslouch package [@bartoszek2024fast]. This approach aligns closest with our goals, however the current maximum-likelihood implementation only allows for Gaussian traits, suffers from estimation problems, and lacks flexibility (such as accounting for latent factors, measurement error, missing data, repeated measures, etc.), which is why what motivated us to pursue a Bayesian implementation in the Stan probabilistic programming language [@Carpenter2017]. 
...
to do so through the development of what we refer to simply as generalized
dynamic phylogenetic models (GDPMs) of trait coevolution. Our GDPM is driven by a 
multivariate stochastic differential equation, similar to a multivariate 
Ornstein-Uhlenbeck (OU) process (see section 2.4. for an overview
of the basic OU model) with greater model flexibility and complexity than is 
permitted by current implementations. Our approach arose from two complementary
...
than assuming piecewise constant, see 2.4.). Second, we 
...
evolution, akin to Pagel's [-@Pagel1994] method and existing OU methods [@bartoszek2012phylogenetic; @bartoszek2024fast] but without restrictions on the
type or number of traits, and with more flexibility to account for the realities of comparative data.
...
value $\theta$ for each trait $\eta_i$ in the GDPM as
...
values for the system as a whole ($\textbf{b}_{\Delta\infty}$), which 
are determined by 
...
\begin{algorithm}
  \caption{Implementation of the generalized dynamic phylogenetic model}\label{alg:alg}
  \DontPrintSemicolon
  \For{$n \in N$}{
...
To aid researchers in applying the GDPM to their own data, we have developed the 
`coevolve` R package (https://github.com/ScottClaessens/coevolve). This 
fully documented package provides a user-friendly interface for applying GDPMs, 
with simple functions for generating GDPM Stan code and data lists, fitting GDPMs
...
are only 20 or 50 species, but it is able to detect this causal effect with 
sufficient power when the sample size is 100. Note, however, that these power
estimates apply only to the generative model in \autoref{eq:generative};
we would expect to find different power estimates when simulating with different
effect sizes and variables with different response distributions.
...
20, one continuous trait and one binary trait) and fitted the GDPM to these
...
of GDPMs. The issues and innovations highlighted here should also generalize to 
many other morphological and socio-ecological traits.

To illustrate the potential of GDPMs to analyze brain size evolution, we used the
...
[@Smaers2021]. Note also that this GDPM used Bayesian imputation rather than 
...
history traits and body size is also well established [@Healy2019; @Wright2019].

A GDPM analyzing the coevolution of these two latent factors (see Supplementary
...
(TSD). A GDPM analyzing the co-evolution of the two latent variables RI and TSD 
...
phylogenetic signals, suggesting that a GDPM could reasonably be used to assess
...
modelled both variables as ordinal in a GDPM. The model revealed that both 
...
# Conclusions and Limitations (ROUGH DRAFT!)

We presented a novel class of phylogenetic methods that allows the detection of coevolutionary dynamics and has several advantages over existing approaches. These include an explicitly dynamic approach, which can detect evolutionary contingencies and reciprocal causation, coupled with the enormous flexibility of a Bayesian implementation in the Stan programming language, allowing researchers to naturally account for latent factors, repeated measures, measurement error, missing data, and other real-life complexities of comparative data. The coevolve R package supports some of these complexities (such as missing data), as well as a reasonable diversity of distributions, facilitating the adoption of this method for most R users. Researchers who might prefer the flexibility of coding their models directly in Stan, can build on the annotated code provided with our worked empirical examples.  

Despite several advances, our method is still subject to several limitations, some specific to GDPMs, and some more broadly applicable. First, in cases where traits experience correlated drift, GDPMs will struggle to separate this effect from cross-selection (EXPAND ON THIS, PERHAPS WITH A REAL BIOLOGICAL EXAMPLE?). Second, GDPMs assume that cross-selection effects are linear; this assumption can be somewhat relaxed by using clade-specific parameters, though the effects will still be 'locally' linear (i.e. within a given clade). Third, as with any Bayesian estimation, GDPMs can be computationally costly, though we note that the Hamiltonian Monte Carlo sampler of Stan is substantially more efficient than conventional MCMC samplers, typically requiring only a few thousand instead of millions of iterations; all the models presented here, including the empirical applications, generally took minutes to hours to run. Fourth, as with all comparative analyses, GDPMs are subject to concerns about data quality and bias; e.g. that extant trait variation reflects past variation, that available phylogenies accurately reflect true evolutionary history, etc. We also note that since GDPMs explicitly work as a time series, inference or knowledge about ancestral states is particularly crucial, and relevant information should be incorporated into the models whenever possible - for instance, Ringen et al [@Ringen_preprint] used priors for low complexity on the last common ancestor of all human societies; in this way, virtual archeology (or paleontology) can be enhanced by real archeology. Lastly, we caution that GDPMs detect contingencies, and not causation per se, though this caveat applies to all approaches to causal inference from observational data [@Pearl2009; @Pagel1994]. 
