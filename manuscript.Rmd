...
\begin{center}
Phylogenetic comparative methods are widely used to study the coevolution of
biological and cultural traits over evolutionary time. Among these methods, 
some of the most popular are phylogenetic generalised linear mixed models, 
phylogenetic path analysis, and Pagel's `discrete' method. While these methods
have clear benefits under specific scenarios, they also have important
limitations, such as unrealistic assumptions about the evolutionary process,
a focus on evolutionary correlation rather than causation, and limits on the 
number and type of traits that researchers can include. Here, we develop a
novel class of Bayesian \textit{generalized dynamic phylogenetic models} (GDPMs) that
overcomes these limitations and synthesizes the strengths of existing 
phylogenetic comparative methods into a single generalizable framework for 
dynamic phylogenetic causal inference. We formally and conceptually introduce
this class of model, which is applicable to any number of traits following any
response distribution. We provide a short tutorial for fitting these models
with the \texttt{coevolve} R package, a user-friendly package purpose-built for 
fitting GDPMs in Stan. We highlight previous studies that have used GDPMs to 
make inferences about trait coevolution, including analyses of brain size 
in non-human primates and social complexity in human societies, and suggest
some directions for future model development and research.
\end{center}
...
We presented 
First paragraph repeats major advances of the method:
Dynamic model, analogous to Pagel’s discrete, but without being limited to binary traits
Can handle any distribution, latent factors
Can detect evolutionary contingencies (A→B, B→A) and reciprocal causation
Unbiased estimator, empirical examples summary

Second paragraph states limitations, starting with specific to this method then going broader
1: Correlated drift cannot be separated from cross-selection (though this is possible with ‘real’ longitudinal data) - biological examples of traits under correlated drift?
2: Cross-selection effects assumed to be linear - this can be somewhat relaxed with clade-specific parameters, but still locally linear
3: Computational cost - as with any Bayesian model - can range from minutes (for small phylogenies / datasets) to hours (SCCS example) or even days (for the most complex models, or if looped over a sample of trees)
4: Interpretation - we’re detecting contingency, not true causality; though this is the same for other kinds of causal inference (Pearl, Pagel, Turchin)
5: Data quality and inference - comparative data tend to be crude, and all comparative analyses assume that extant variation reflects all past variation. Virtual archeology / paleontology is not as good as real archeology / paleontology! This can be somewhat amended by including information about ancestral nodes, e.g. complexity SCCS analysis put prior for low complexity on last common ancestor - in principle this could be done for any internal node, with prior reflecting exact state of knowledge
