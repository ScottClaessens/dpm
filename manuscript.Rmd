...
novel class of Bayesian *dynamic phylogenetic models* (DPMs) that overcomes
these limitations and synthesizes the strengths of existing phylogenetic 
comparative methods into a single generalizable framework for dynamic
phylogenetic causal inference. We formally and conceptually introduce this
class of model, which is applicable to any number of traits following any
response distribution. We provide a short tutorial for fitting these models
with the `coevolve` R package, a user-friendly package purpose-built for 
