...
We presented a novel class of phylogenetic methods that allows the detection of coevolutionary dynamics and has several advantages over existing approaches. These include an explicitly dynamic approach, which can detect evolutionary contingencies and reciprocal causation, coupled with the enormous flexibility of the Bayesian implementation in the Stan programming language, allowing researchers to naturally account for latent factors, repeated measures, measurement error, missing data, and other real-life complexities of comparative data. The coevolve R package supports some of these complexities (such as missing data), as well as a reasonable diversity of distributions, facilitating the adoption of this method for most R users, while some researchers might prefer the flexibility of coding their models directly in Stan.  

Despite several advances, our method is still subject to several limitations, some specific to GDPMs, and some more broadly applicable. In cases where traits experience correlated drift, GDPMs will struggle to separate this effect from cross-selection (EXPAND ON THIS, PERHAPS WITH A REAL BIOLOGICAL EXAMPLE?). GDPMs assume that cross-selection effects are linear; this assumption can be somewhat relaxed by using clade-specific parameters, though the effects will still be 'locally' linear (i.e. within a given clade). As with any Bayesian estimation, GDPMs can be computationally costly, though we note that the Hamiltonian Monte Carlo sampler of Stan is substantially more efficient than conventional MCMC samplers, typically requiring only a few thousand instead of millions of iterations; the models presented in this paper generally took minutes to hours to run. As with all comparative analyses, GDPMs are subject to concerns about data quality and bias; e.g. that extant trait variation reflects past variation, that available phylogenies accurately reflect true evolutionary history, etc. We also note that since GDPMs work as a time series, inference or knowledge about ancestral states is particularly crucial, and relevant information should be incorporated into the models - for instance, Ringen et al [@Ringen_preprint] used priors for low complexity on the last common ancestor of all human societies; in this way, virtual archeology (or paleontology) can be enhanced by real archeology. Lastly, we caution that GDPMs detect contingencies, and not causation per se, though this caveat applies to all approaches to causal inference from observational data [@Pearl2009; @Pagel1994]. 
